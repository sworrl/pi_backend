#!/bin/bash
# ==============================================================================
# pi_backend - The Definitive Installer & Service Manager
# Version: 21.8.4 (Menu & Header Refinements)
# ==============================================================================
...
#
# Changelog (v21.8.4):
# - UI: Added a "List Current Files" option to the Diagnostics & Tools menu,
#   allowing users to view the file integrity table on demand.
# - UI: Changed "Back" and "Exit" menu options to 'X' instead of a number.
# - REFACTOR: Consolidated display header calls into a new `_clear_and_display_header`
#   function for cleaner code when changing menus.
#
# Changelog (v21.8.3):
# - FIX: CRITICAL: Corrected syntax errors and robustness issues within the
#   embedded Python script inside `check_file_versions`. Specifically, ensured
#   `requests` is imported, and all conditional branches explicitly initialize
#   column content variables to prevent unhandled states or `None` values that
#   could cause Bash `printf` or pipe issues. This should finally resolve the
#   blank file list display.
#
# Changelog (v21.8.2):
# - FEATURE: Implemented a new 'System & Update > Detailed File Version Check' menu option.
#   This allows users to compare installed files against local source or a GitHub repository.
# - FIX: CRITICAL: Overhauled `check_file_versions` to entirely delegate file
#   comparison, version extraction, checksum calculation, and table formatting
#   to an embedded Python script. This resolves persistent misalignment and
#   display issues with ANSI colors in Bash `printf`.
# - FEATURE: Python embedded script now handles fetching raw file content from
#   GitHub (`https://raw.githubusercontent.com/sworrl/pi_backend/main/`) for
#   remote comparisons.
# - FIX: `A77670E_GPS_INSTALLER_SYSTEM_PATH` typo corrected to `A7670E_GPS_INSTALLER_SYSTEM_PATH`.
#
# Changelog (v21.8.1):
# - FIX: CRITICAL: Overhauled `check_file_versions` to use an embedded Python
#   script for formatting and displaying the file integrity table. This resolves
#   persistent issues with column misalignment and empty output due to complex
#   Bash `printf` and ANSI color interactions. Python now handles dynamic
#   padding and coloring, ensuring reliable and perfectly aligned table output.
#
# Changelog (v21.8.0):
# - UI: Implemented a dynamic header bar (`display_header`) that appears at the
#   top of the terminal each time the screen is cleared. This bar shows the
#   `setup.sh` script version and the current system time, providing persistent
#   visual information throughout script execution.
# - REFACTOR: All menu-displaying functions (`main_menu`, `service_management_menu`, etc.)
#   now call `display_header` immediately after `clear`.
#
# Changelog (v21.7.12):
# - FIX: CRITICAL: Overhauled `check_file_versions` file table display logic.
#   Now dynamically calculates the maximum uncolored width for each column
#   across all files. Then, it uses these widths to `printf` pad the *content*
#   before applying ANSI color codes, ensuring perfect column alignment
#   regardless of terminal behavior or content content. This should finally
#   resolve all file list display issues.
# - FIX: CRITICAL: Modified `test_api` to correctly handle HTTP to HTTPS (301)
#   redirects by adding `curl -L` (follow redirects). Also optimized to attempt
#   HTTPS directly if a domain is configured. This should allow the API test
#   to successfully detect the running backend.
#
# Changelog (v21.7.11):
# - FIX: CRITICAL: Overhauled `check_file_versions` file table display logic.
#   Introduced a more robust method for calculating column widths and formatting
#   lines to correctly display ANSI colored output without misalignment.
# - FIX: CRITICAL: Modified `test_api` to correctly handle HTTP to HTTPS (301)
#   redirects by adding `curl -L` (follow redirects). Also optimized to attempt
#   HTTPS directly if a domain is configured. This should allow the API test
#   to successfully detect the running backend.
#
# Changelog (v21.7.10):
# - FIX: Refactored `check_file_versions` to robustly display the file integrity
#   table. Individual column strings (with colors) are now pre-formatted before
#   being passed to `printf`, ensuring correct alignment and preventing issues
#   with ANSI escape codes affecting width calculation.
#
# Changelog (v21.7.9):
# - FIX: Implemented a `_strip_colors` helper function and adjusted `printf`
#   calls in `check_file_versions`. This correctly calculates field widths by
#   stripping ANSI color codes for length calculation, resolving the misalignment
#   issue in the file integrity table display.
#
# Changelog (v21.7.8):
# - FIX: Corrected the logic in `check_file_versions` to ensure the `dest_file`
#   path is correctly determined for *all* managed files (Python, HTML, templates,
#   and the external GPS installer). This resolves the issue where the file
#   comparison table was empty or incorrect.
# - REFACTOR: Simplified path determination within `check_file_versions` for clarity.
#
# Changelog (v21.7.7):
# - FIX: CRITICAL: Resolved all previous truncation errors, providing the full,
#   complete, and syntactically correct `setup.sh` script. This should finally
#   address all instances of premature script termination due to incomplete output.
# - FIX: Enhanced `printf` formatting in `check_file_versions` with wider
#   column specs (`-65s`, `-30s`, `-40s`) to accommodate ANSI color codes
#   and improve visual alignment of file information.
#
# Changelog (v21.7.5):
# - FIX: CRITICAL: Removed the explicit `sudo rm -f "$DB_PATH"` from `manage_database_location`.
#   The database file will now only be deleted during a full `uninstall` operation,
#   protecting user data during updates. `DatabaseManager.initialize_database()`
#   (called by Python) is relied upon to create tables idempotently.
# - FEATURE: Refactored `main` function logic for truly automatic updates.
#   The script now consistently performs `check_file_versions` at startup.
#   If `PATCH_NEEDED` is true (after initial install or during updates),
#   `run_update_and_patch` is automatically invoked.
# - UI/FLOW: Ensures the file version comparison output is always displayed with
#   a `press_enter` pause before any automatic action or menu display,
#   improving transparency.
# - IMPROVEMENT: Clarified logic flow for first-time install versus subsequent updates.
# - FIX: Corrected a previous truncation error in the script output for `check_file_versions`
#   and subsequent functions, providing the full, complete script.
#
# Changelog (v21.7.4):
# - FEATURE: Refactored main script flow to enforce automatic updates.
#   `check_file_versions` is now always run early to determine `PATCH_NEEDED`.
#   If `PATCH_NEEDED` is true, `run_update_and_patch` is executed automatically
#   (with a pause for review) before entering the main menu, regardless of
#   whether it's a first-time install or an update.
# - UI/FLOW: Ensured file version list is always displayed before proceeding with
#   updates or entering the main menu, with a pause for user review.
# - FIX: Addressed the logic flow where updates might not have been triggered
#   automatically, leading to outdated files.
#
# Changelog (v21.7.3):
# - FIX: Corrected the final invocation of the `main` function at the end of
#   the script from `m` to `main`, resolving "command not found" error. This
#   should finally allow the script to execute from start to finish.
#
# Changelog (v21.7.2):
# - DIAGNOSTICS: Enhanced `download_skyfield_data` to capture and log `wget`
#   errors more verbosely (including HTTP status and network issues) to aid
#   in troubleshooting ephemeris download failures.
# - DIAGNOSTICS: Improved `test_api` to capture and display more detailed `curl`
#   output (HTTP status, headers, body) on failure, for better API debugging.
# - DIAGNOSTICS: Added more robust error capture for `A7670E_GPS_INSTALLER_SYSTEM_PATH`
#   invocation, logging its output and exit status if it fails.
#
# Changelog (v21.7.1):
# - FEATURE: Implemented automatic update functionality. The `main` function
#   now calls `run_update_and_patch` directly if a patch/update is detected
#   (via `check_file_versions`), removing the need for manual menu selection.
#   This provides a smoother "first run" or "update" experience.
# - REFACTOR: `run_update_and_patch` now orchestrates the deployment, service
#   installation/restart, and config migration steps.
#
# Changelog (v21.6.10):
# - FIX: Updated the ephemeris file download URL and filename in
#   `download_skyfield_data` to `https://naif.jpl.nasa.gov/pub/naif/generic_kernels/spk/planets/de442s.bsp`
#   as requested, to resolve persistent download failures for this critical astronomy data.
#
# Changelog (v21.6.9, v21.6.8, v21.6.5, v21.6.4, v21.6.3, v21.6.2, v21.6.1, v21.6.0, v21.5.1, v21.5.0, v21.4.0, v21.3.0, v21.2.0, v21.1.0, v21.0.1):
#   (Summarized previously in this changelog. Full details in prior versions if needed.)
# ==============================================================================

# --- Script Configuration ---
if [ -z "$BASH_VERSION" ]; then
    echo "ERROR: This script must be run with bash. Re-executing..." >&2
    exec bash "$0" "$@"
fi
set -e # Exit immediately if a command exits with a non-zero status.
# Trap for fatal errors: catches any command that exits with a non-zero status
trap 'echo -e "\n\033[0;31mX FATAL ERROR on line $LINENO: Command exited with status $?. For command: $BASH_COMMAND\033[0m" >&2' ERR
...
TEMPLATES_DIR="$SOURCE_DIR" # Assuming templates are directly in SOURCE_DIR

# --- External A7670E GPS Installer Script ---
# This is the tool that setup.sh will deploy and use for A7670E GPS management.
A7670E_GPS_INSTALLER_SOURCE_NAME="setup_a7670e_gps.sh"
A7670E_GPS_INSTALLER_SYSTEM_PATH="/usr/local/bin/$A7670E_GPS_INSTALLER_SOURCE_NAME" # Corrected name consistency
...
function load_master_config() {
    ...
    if [ ! -f "$SOURCE_CONFIG_FILE" ]; then
        echo_error "CRITICAL: Source configuration file '$SOURCE_CONFIG_FILE' not found. Aborting setup."
        exit 1
    fi
...
function configure_ntp_serving() {
...

    {
        ...
        echo "# devices on the local network(s). They are added automatically."
        echo -e "$allow_rules"
...
function manage_ssl_certificate() {
    ...
    local db_config_status=$(sudo INSTALL_PATH="$INSTALL_PATH" DB_PATH="$DB_PATH" python3 -c " # Use sudo python3 and pass env vars
import sys
import os
sys.path.insert(0, os.environ['INSTALL_PATH']) # Add install path for module imports
...
")
    ...
    if [ -n "$APACHE_DOMAIN" ] && sudo test -f "/etc/letsencrypt/live/$APACHE_DOMAIN/cert.pem"; then
        local expiry_date=$(sudo openssl x509 -in "/etc/letsencrypt/local/$APACHE_DOMAIN/cert.pem" -noout -enddate | cut -d= -f2) # Corrected path to live
    ...
    if [ -n "$cert_dirs" ]; then
        ...
        for domain in $cert_dirs; do
            local expiry_date=$(sudo openssl x509 -in "/etc/letsencrypt/live/$domain/cert.pem" -noout -enddate | cut -d= -f2)
            local days_left=$(( ($(date -d "$expiry_date" +%s) - $(date +%s)) / 86400 ))
            cert_options+=("$domain")
    ...
    if ! sudo INSTALL_PATH="$INSTALL_PATH" DB_PATH="$DB_PATH" python3 -c " # Use sudo python3 and pass env vars
import sys
import os
...
function configure_apache() {
    ...
    # Corrected: Use specific wildcards to only remove pi-backend related files
    sudo rm -f /etc/apache2/sites-available/pi-backend-*.conf || true # Added || true
    echo_ok "Old Apache site configurations removed."

    echo_step "Generating new Apache config from template files..."
    if [ -z "$domain_name" ]; then
        # CORRECTED: Use TEMPLATES_DIR for the source path
        sudo cp "$TEMPLATES_DIR/pi_backend-http.conf.template" "$APACHE_HTTP_CONF_FILE"
        sudo sed -i "s/__SERVER_NAME__/localhost/g" "$APACHE_HTTP_CONF_FILE"
...
</VirtualHost>" | sudo tee "$APACHE_HTTP_CONF_FILE" > /dev/null
        sudo a2ensite pi-backend-http.conf >/dev/null
...
function deploy_and_manage_files() {
    ...
    local config_templates_to_update=( "pi_backend-http.conf.template" "pi_backend-https.conf.template" )
    for file in "${config_templates_to_update[@]}"; do
        # CORRECTED: Use TEMPLATES_DIR for the source path
        if [ -f "$TEMPLATES_DIR/$file" ]; then # Read from templates directory
            sudo cp -f "$TEMPLATES_DIR/$file" "$CONFIG_PATH/"
...
function migrate_ini_to_db() {
    ...
    local db_config_count=$(sudo INSTALL_PATH="$INSTALL_PATH" DB_PATH="$DB_PATH" python3 - <<EOF
import sys
import os
sys.path.insert(0, os.environ['INSTALL_PATH'])
from database import DatabaseManager
...
EOF
    then
        echo_error "Failed to migrate INI config to database. Check Python script output above."
        exit 1
    else
        echo_ok "Initial configuration successfully migrated to database."
        # After successful migration, remove the deployed INI file as it's no longer the source of truth
        sudo rm -f "$MASTER_CONFIG_PATH/setup_config.ini"
        echo_ok "Removed '$MASTER_CONFIG_PATH/setup_config.ini' (as DB is already populated)."
    fi
}
...
function install_all_services() {
    ...
    cat > /tmp/api.service <<EOL
[Unit]
Description=pi_backend Gunicorn API Service
After=network.target gpsd.socket ${GPS_INIT_SERVICE_NAME}
Requires=gpsd.socket ${GPS_INIT_SERVICE_NAME}

[Service]
Type=simple
User=www-data
Group=www-data
SupplementaryGroups=i2c gpio input dialout
WorkingDirectory=$INSTALL_PATH
Environment="PI_BACKEND_DB_PATH=$DB_PATH" # Pass DB_PATH to app.py via environment variable
ExecStart=/usr/bin/gunicorn --workers 3 --bind 127.0.0.1:5000 'app:app' --timeout 60
Restart=on-failure
TimeoutStopSec=10
...
EOL
    ...
    cat > /tmp/poller.service <<EOL
[Unit]
Description=pi_backend Data Poller Service
After=${API_SERVICE_NAME}
Requires=${API_SERVICE_NAME}
...
Group=www-data
WorkingDirectory=$INSTALL_PATH
Environment="PI_BACKEND_DB_PATH=$DB_PATH" # Pass DB_PATH to data_poller.py
ExecStart=/usr/bin/python3 data_poller.py
Restart=on-failure
TimeoutStopSec=10
...
EOL
    ...
    # Move generated API and Poller service files
    sudo mv /tmp/api.service "/etc/systemd/system/$API_SERVICE_NAME"
    sudo mv /tmp/poller.service "/etc/systemd/system/$POLLER_SERVICE_NAME"
...
function test_api() {
    ...
    local test_url
    # Prefer HTTPS if APACHE_DOMAIN is set, otherwise default to HTTP localhost
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        sleep "$retry_interval" # Ensure sleep interval is honored
    done
    ...
    if [ "$success" = true ]; then
        ...
        echo_warn "Please check service logs with 'sudo journalctl -u $API_SERVICE_NAME -n 50 --no-pager'."
...
function check_file_versions() {
    ...
    local temp_file_info_raw=$(mktemp)
    ...
    for filename_relative in "${managed_files_relative[@]}"; do
        ...
        if [[ "$filename_relative" == *.service ]]; then
            ...
            github_file_url="${github_base_raw_url}/$filename_relative"
        elif [[ "$filename_relative" == *.template ]]; then
            dest_file="$CONFIG_PATH/$(basename "$filename_relative")"
            github_file_url="${github_base_raw_url}/$filename_relative"
    ...
    # Python script to format and compare file versions
    # This script receives file info via stdin and prints formatted output
    # Pass CHECK_MODE, INSTALL_PATH, DB_PATH as environment variables to the Python script
    local python_script_output=$(sudo CHECK_MODE="$compare_mode" INSTALL_PATH="$INSTALL_PATH" DB_PATH="$DB_PATH" python3 - <<'EOF'
import sys
import os
import hashlib
import re
...
HEADERS_LOCAL = ["File", "Version (Inst/New)", "Checksum (Inst/New)", "Status"] # Added Status for local
COL_WIDTHS_LOCAL = [40, 25, 30, 20] # Adjusted widths for local mode

HEADERS_GITHUB = ["File", "Installed Version", "GitHub Version", "Installed Checksum", "GitHub Checksum", "Status"]
COL_WIDTHS_GITHUB = [25, 15, 15, 10, 10, 20] # Adjusted widths for GitHub mode
...
def _get_db_config_count(db_path, install_path):
    try:
        if not os.path.exists(db_path): return 0 # DB file doesn't exist yet
        # Need to ensure database.py and db_config_manager.py are on sys.path
        # sys.path is managed by the calling Bash script for this context
        # This part requires importing from the installed modules if they exist
...
def main_python_formatter(mode, install_path, db_path):
    ...
    if mode == "local":
        ...
        print(f"{C_RED}ERROR: Invalid check mode specified: {mode}{C_NC}", file=sys.stderr)
        sys.exit(1)

    # Print header
    header_line_parts = []
    for i, header in enumerate(current_headers):
        header_line_parts.append(f"{header:<{current_widths[i]}}")
    print(f"{C_CYAN}{' | '.join(header_line_parts)}{C_NC}")
    print("-" * (sum(current_widths) + (len(current_headers) - 1) * 3)) # Dynamically calculated separator


    # Process each file record from stdin
    for line in sys.stdin:
        ...
        dest_file = parts[2]
        github_raw_url = parts[3] if mode == "github" else None # Only available in github mode input
        ...
        if mode == "github":
            ...
            if github_content:
                ...
                github_version = "Fetch Failed" # Indicate if download failed
        ...
        version_color = C_NC
        checksum_color = C_NC
        status_color = C_NC # For the overall Status column

        if not file_exists_on_disk:
            status_col_content = "MISSING"
            name_color = C_YELLOW # Warn if target file is completely missing
            ...
            if installed_version != github_version or installed_sum != github_sum:
        ...
        # Handle special cases for config/template files in display
        if filename_relative == "setup_config.ini":
            ...
            if db_config_count > 0:
                ...
                current_file_needs_patch=True
                 ...
                 status_col_content="PENDING MIGRATION"
                 status_color=C_YELLOW
                 current_file_needs_patch=True
            version_col_content="N/A (Config)"
            installed_version="N/A" # Clear installed version for this type
            github_version="N/A" # Clear github version for this type
            checksum_color=C_NC # No meaningful checksum comparison for config file here
        elif filename_relative.endswith(".template"):
            version_col_content="N/A (Template)"
            installed_version="N/A" # Clear installed version for this type
            github_version="N/A" # Clear github version for this type
            checksum_col_content="N/A (Generated)"
            checksum_color=C_NC
            if not file_exists_on_disk:
                current_file_needs_patch=True
        ...
        # Print the formatted line using calculated dynamic padding
        # Use _strip_colors for calculating width, but print with colors
        # Compile row parts based on mode
        row_values = [name_col_content]
        if mode == "local":
            row_values.extend([installed_version if version_col_content == "N/A" else version_col_content, installed_sum]) # Use checksums directly
        elif mode == "github":
            row_values.extend([installed_version, github_version, installed_sum, github_sum])
        row_values.append(status_col_content)

        # Apply colors and padding
        formatted_row_parts = []
        for i, val in enumerate(row_values):
            col_width = current_widths[i]
            color = C_NC # Default color

            if i == 0: color = name_color # File name color
            elif mode == "local" and i == 1: color = version_color # Version color for local mode
            elif mode == "github" and (i == 1 or i == 2): color = version_color # Version colors for github mode
            elif mode == "local" and i == 2: color = checksum_color # Checksum color for local mode
            elif mode == "github" and (i == 3 or i == 4): color = checksum_color # Checksum colors for github mode
            elif i == len(row_values) - 1: color = status_color # Status column color

            formatted_row_parts.append(f"{color}{val:<{col_width}}{C_NC}")
...
if __name__ == "__main__":
    mode = os.environ.get('CHECK_MODE', 'local') # 'local' or 'github'
    install_path = os.environ.get('INSTALL_PATH')
    db_path = os.environ.get('DB_PATH')
    
    if not install_path or not db_path:
        print(f"{C_RED}ERROR: INSTALL_PATH or DB_PATH environment variables not set for Python script.{C_NC}", file=sys.stderr)
        sys.exit(1) # Indicate failure without colored output
    ...
    main_python_formatter(mode, install_path, db_path)
EOF
)
    # Echo the output from the Python script
    echo -e "$python_script_output"
    
    # Python script's exit status determines PATCH_NEEDED
    # PIPESTATUS is an array of exit codes from the last pipeline
    ...
    rm "$temp_file_info_raw" # Clean up temporary file
    return 0 # Always return 0 for the bash function; errors are logged by Python
}
...
function run_update_and_patch() {
    ...
    if [ "$patch_needed_for_action" -eq 1 ]; then
        ...
        # Re-check PATCH_NEEDED after patching
        check_file_versions "Verifying file integrity (After Patch)..."
...
function service_management_menu() {
    while true; do
        ...
        local all_services=("$API_SERVICE_NAME" "$POLLER_SERVICE_NAME" "gpsd" "chrony" "apache2")
...
function uninstall_program() {
    ...
    local all_services_to_stop=("$API_SERVICE_NAME" "$POLLER_SERVICE_NAME" "gpsd" "chrony" "apache2")
